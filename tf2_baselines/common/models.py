...
from tf2_baselines.a2c import utils
from tf2_baselines.a2c.utils import conv, fc, conv_to_fc, batch_to_seq, seq_to_batch
...
def nature_cnn(unscaled_images, **conv_kwargs):
    ...
    h = activation(conv(scaled_images, 'c1', nf=32, rf=8, stride=4, init_scale=np.sqrt(2), **conv_kwargs))
    h2 = activation(conv(h, 'c2', nf=64, rf=4, stride=2, init_scale=np.sqrt(2), **conv_kwargs))
    h3 = activation(conv(h2, 'c3', nf=64, rf=3, stride=1, init_scale=np.sqrt(2), **conv_kwargs))
    h3 = conv_to_fc(h3)
    return activation(fc(h3, 'fc1', nh=512, init_scale=np.sqrt(2)))
...
def get_network_builder(name):
    ...
    Usage Example:
    -------------
    from tf2_baselines.common.models import register
