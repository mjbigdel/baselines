...
def conv2d(x, num_filters, name, filter_size=(3, 3), stride=(1, 1), pad="SAME", dtype=tf.float32, collections=None, summary_tag=None):
    ...
    w = tf.Variable(name="W", filter_shape=filter_shape, dtype=dtype, initial_value= tf.initializers.random_uniform(-w_bound, w_bound),
                        collections=collections)
    b = tf.Variable(name="b", filter_shape=[1, 1, 1, num_filters], initial_value=tf.initializers.zeros(),
                        collections=collections)

    if summary_tag is not None:
        tf.summary.image(summary_tag, tf.transpose(tf.reshape(w, [filter_size[0], filter_size[1], -1, 1]), [2, 0, 1, 3]), max_outputs=10)
...
class SetFromFlat(object):
    ...
    def set_from_flat(self, theta):
        ...
        self.theta = theta = tf.keras.layers.Input(shape=[total_size], dtype=self.dtype)
        # self.theta = theta = tf.placeholder(dtype, [total_size])
        start = 0
        assigns = []
        for (shape, v) in zip(shapes, self.var_list):
            size = intprod(shape)
            assigns.append(tf.Variable.assign(v, tf.reshape(theta[start:start + size], shape)))
            start += size
...
class GetFlat(object):
    ...
    @tf.function
    def get_flat(self):
        op = tf.concat(axis=0, values=[tf.reshape(v, [numel(v)]) for v in self.var_list])
        return op
...
def get_placeholder(name, dtype, shape):
    ...
    out = tf.keras.layers.Input(dtype=dtype, shape=shape, name=name)
...
def load_state(fname, sess=None):
    ...
    return tf.keras.models.load_model(filepath=fname)
...
def save_state(fname, sess=None):
    ...
    tf.keras.models.save_model(filepath=fname)
